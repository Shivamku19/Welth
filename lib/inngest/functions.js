      ...
      await step.run(`generate-report-${user.id}`, async () => {
        ...
        if (transactions.length > 0) {
          ...
          const prompt = `
            ...
            Transactions:
            ${transactions.map((t) => `${t.date.toISOString().split("T")[0]}: $${t.amount} for ${t.category} (${t.type})`).join("\n")}

            Return the insights as a single string of 3 bullet points. No markdown formatting, just plain text bullets starting with a dash or bullet symbol.
          `;
        ...
        const statData = categoryStats.map((stat) => ({
